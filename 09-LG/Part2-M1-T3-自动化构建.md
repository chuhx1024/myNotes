...
### 自动化构建工具
- Grunt: 最早的前端构建系统 生态很完善 几乎可以帮你完成任何想做的事 工作过程基于临时文件(有磁盘读写操作)  构建过程很慢
- Gulp:  构建速度快  因为他是基于内存处理的 支持同时支持多个任务 使用方式直观易懂 生态很完善  后来居上  目前前端最流行的构建系统
